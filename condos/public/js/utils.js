...
frappe.provide("condos");
frappe.provide("condos.utils");
...
$.extend(condos, {
	...
	hide_company: function() {
		if(cur_frm.fields_dict.company) {
			...
			} else if(condos.last_selected_company) {
				if(!cur_frm.doc.company) cur_frm.set_value("company", condos.last_selected_company);
			}
		}
	},
...
$.extend(condos.utils, {
	...
	get_terms: function(tc_name, doc, callback) {
		if(tc_name) {
			return frappe.call({
				method: 'condos.settings.doctype.terms_and_conditions.terms_and_conditions.get_terms_and_conditions',
				args: {
	...
	make_subscription: function(doctype, docname) {
		frappe.call({
			method: "condos.accounts.doctype.subscription.subscription.make_subscription",
			args: {
				doctype: doctype,
				docname: docname
			},
...
condos.utils.map_current_doc = function(opts) {
