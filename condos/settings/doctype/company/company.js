...
frappe.provide("condos.company");

frappe.ui.form.on("Company", {
	setup: function(frm) {
		condos.company.setup_queries(frm);
	},
	...
	refresh: function(frm) {
		...
		condos.company.set_chart_of_accounts_options(frm.doc);
	...
	delete_company_transactions: function(frm) {
		frappe.verify_password(function() {
			...
			function(data) {
				...
				frappe.call({
					method: "condos.settings.doctype.company.delete_company_transactions.delete_company_transactions",
					args: {
...
condos.company.set_chart_of_accounts_options = function(doc) {
	var selected_value = doc.chart_of_accounts;
	if(doc.country) {
		return frappe.call({
			method: "condos.financials.doctype.account.chart_of_accounts.chart_of_accounts.get_charts_for_country",
			args: {
				"country": doc.country,
				"with_standard": true
			},
...
cur_frm.cscript.change_abbr = function() {
	...
	dialog.fields_dict.update.$input.click(function() {
		...
		return frappe.call({
			method: "condos.settings.doctype.company.company.enqueue_replace_abbr",
			args: {
...
condos.company.setup_queries = function(frm) {
	$.each([
		...
		condos.company.set_custom_query(frm, v);
	});
...
condos.company.set_custom_query = function(frm, v) {
	var filters = {
